...
def prenorm_block_causal_lact_swiglu(
    ...
    for i in range(0, seq_len - chunk_size, chunk_size):
        ...
        w0_main = w0_main + dw0 if clip_grad_norm is None else w0_main + dw0 * torch.clamp(clip_grad_norm / (dw0.norm(dim=(1, 2), keepdim=True) + 1e-5), max=1.0)
        w1_main = w1_main + dw1 if clip_grad_norm is None else w1_main + dw1 * torch.clamp(clip_grad_norm / (dw1.norm(dim=(1, 2), keepdim=True) + 1e-5), max=1.0)
        w2_main = w2_main + dw2 if clip_grad_norm is None else w2_main + dw2 * torch.clamp(clip_grad_norm / (dw2.norm(dim=(1, 2), keepdim=True) + 1e-5), max=1.0)
